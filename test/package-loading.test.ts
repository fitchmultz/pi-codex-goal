...
import { InMemoryCredentialStore, validateToolArguments, type JsonValue } from "@earendil-works/pi-ai";
import {
  createAgentSession,
...
async function checkPackage(root: string, packageRoot: string, extension: ".ts" | ".js"): Promise<void> {
  ...
  try {
    ...
    const validateCreate = (args: Record<string, JsonValue>) => validateToolArguments(createGoal, {
      type: "toolCall", id: "create", name: "create_goal", arguments: args,
    });
    const rejectBudget = async (args: Record<string, JsonValue>) => {
